...
const Income = () => {
  ...
  // Solved Time Complexity from O(n^2) to O(n)
  const map = new Map();
  ...
  data.forEach((d) => {
    ...
    if (timeFrame === "daily") {
      ...
      key = `${d.date.getDate()}-${d.date.getMonth()}`;
    }
  ...
  filteredTransactions.forEach((transaction) => {
    ...
    if (timeFrame === "daily") {
      key = transDate.getHours();
    } else if (timeFrame === "yearly") {
      key = transDate.getMonth();
    } else {
      key = `${transDate.getDate()}-${transDate.getMonth()}`;
    }
  ...
  const fetchOverview = useCallback(
    async (range = timeFrame ?? "monthly") => {
      try {
        const res = await axios.get(`${API_BASE}/api/transaction/overview`, {
  ...
  const handleAddTransaction = useCallback(async () => {
    ...
    try {
      ...
      await axios.post(`${API_BASE}/api/transaction/add`, payload, {
  ...
  const handleEditTransaction = useCallback(async () => {
    ...
    try {
      ...
      await axios.put(`${API_BASE}/api/transaction/update/${editingId}`, payload, {
        headers: { "Content-Type": "application/json", ...getAuthHeaders() },
      });
      ...
      const serverMsg = err?.response?.data?.message;
      toast.error(serverMsg || "Server Error While Updating");
    } finally {
      setLoading(false);
    }
  }, [
  ...
  const handleDeleteTransaction = useCallback(
    async (id) => {
      ...
      try {
        setLoading(true);
        await axios.delete(`${API_BASE}/api/transaction/delete/${id}`, {
          headers: getAuthHeaders(),
        });
        ...
        const serverMsg = err?.response?.data?.message;
        toast.error(serverMsg || "Error in Deleting");
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleExport = useCallback(async () => {
    try {
      const res = await axios.get(`${API_BASE}/api/transaction/downloadexcel`, {
        headers: getAuthHeaders(),
        responseType: "blob",
      });
